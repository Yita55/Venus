...
class SettingDataSource:NSObject,UITableViewDataSource{
    ...
    open func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if section == 1 {
            return sections[section].cellContents.count - 1
        }
        return sections[section].cellContents.count
    }

    open func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        return PropertyUtils.readLocalizedProperty(sections[section].header)
    }
    ...
    open func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell{
        ...
        var content = sections[indexPath.section].cellContents[indexPath.row]
        print("cellContents' title=\(content.getTitle())!")
        if indexPath.section == 1 && indexPath.row >= 6 {
            content = sections[indexPath.section].cellContents[indexPath.row + 1]
        }
